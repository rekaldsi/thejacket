    ...
    <div className="space-y-12">
      ...
      <section className="bg-jacket-gray px-6 py-5">
        <div className="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
            ...
            <div>
              ...
              <div className="mt-2 flex flex-wrap items-center gap-2">
                ...
                {candidate.data_status === "limited" && (
                  <span className="rounded-full bg-zinc-700 px-2 py-0.5 text-xs text-zinc-400">{d.candidate_data_status_limited}</span>
                )}
              </div>
              <div className="mt-3">
              ...
              {isUncontested && candidate.years_in_office ? (
                <p className="mt-2 font-mono text-xs uppercase tracking-wider text-zinc-500">
                  {d.candidate_year_in_office}{" "}
                  <span className="text-red-400">{candidate.years_in_office}</span>{" "}
                  {d.candidate_in_office} — {d.candidate_running_unopposed}
                </p>
              ) : null}
              ...
              <div className="mt-3 flex flex-wrap gap-3">
                ...
                {candidate.jacket.fec_id ? (
                  ...
                  >
                    ↗ {d.candidate_fec_filing} ({candidate.jacket.fec_id})
                  </a>
                ) : null}
                {candidate.jacket.ilsbe_id ? (
                  <a
                    ...
                    className="text-xs text-zinc-400 hover:text-jacket-amber hover:underline"
                  >
                    ↗ {d.candidate_ilsbe_filing} ({candidate.jacket.ilsbe_id})
                  </a>
                ) : null}
              </div>
            </div>
          ...
          <div className="text-left lg:text-right">
            <p className="font-mono text-xs uppercase tracking-widest text-zinc-500">{d.candidate_raised_label}</p>
      ...
      {candidate.data_status === "limited" ? (
        <section className="border-l-4 border-zinc-600 bg-zinc-900/50 px-5 py-4">
          <p className="text-sm text-zinc-400">
            <span className="font-mono font-bold text-zinc-300">{d.candidate_limited_data}</span>{" "}
            {candidate.data_note || d.candidate_limited_data_default}
          </p>
        </section>
      ) : null}
        ...
        <section className="border-l-4 border-red-700 bg-red-950/20 px-5 py-4">
          <p className="text-base italic leading-relaxed text-zinc-300">
            {candidate.id === "thomas-dart" ? (
              ...
              <>
                {candidate.name} {d.candidate_uncontested_lede_default} {candidate.office}.{" "}
                {candidate.years_in_office ? `${candidate.years_in_office} ${d.candidate_uncontested_lede_years} ` : ""}
                {d.candidate_no_challenger}
              </>
            )}
          </p>
        </section>
      ...
      {candidate.red_flags.length > 0 ? (
        <section id="red-flags" className="space-y-3">
          <h2 className="text-2xl font-black uppercase text-jacket-red">{d.candidate_red_flags_header}</h2>
      ...
      <section className="grid gap-8 md:grid-cols-2">
        <article className="space-y-3">
          <h2 className={sectionHeader}>{d.candidate_bio_header}</h2>
          <p className="text-sm leading-relaxed text-zinc-300">{candidate.bio}</p>
          <p className="text-xs text-zinc-400">{d.candidate_prior_office} {candidate.prior_office || d.candidate_prior_office_na}</p>
        </article>

        <article className="space-y-3">
          <h2 className={sectionHeader}>{d.candidate_key_votes_header}</h2>
          {candidate.key_votes.length > 0 ? (
            ...
            <p className="text-sm text-zinc-400">{d.candidate_no_votes}</p>
          )}
        </article>
      </section>
      ...
      {candidate.jail_timeline && candidate.jail_timeline.length > 0 ? (
        <section className="space-y-4">
          <h2 className="border-l-2 border-jacket-amber pl-3 text-2xl font-black uppercase tracking-tight text-zinc-100">
            {d.candidate_the_record}
          </h2>
          <p className="text-sm text-zinc-500">{d.candidate_record_subtitle}</p>
