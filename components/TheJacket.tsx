...
export default function TheJacket({ candidateName, totalRaised, donors, sourceCitation, donorsNote }: TheJacketProps) {
  ...
  const listedTotal = donorsWithAmounts.reduce((sum, donor) => sum + (donor.amount as number), 0);
  ...
  return (
    <section className="space-y-6 border border-jacket-border p-4">
      <div className="space-y-2">
        ...
        <p className="text-sm text-zinc-400">{d.jacket_who_funds} {candidateName}?</p>
        {totalRaised === null ? (
          <p className="font-mono text-5xl text-zinc-500">{d.jacket_data_pending}</p>
        ) : (
          <MoneyAmount value={totalRaised} className="text-5xl" />
        )}
        <p className="text-xs text-zinc-600">{d.jacket_source_label} {sourceCitation || d.jacket_source_default}</p>
      </div>

      {donorsWithAmounts.length === 0 ? (
        <div className="border border-dashed border-zinc-800 p-4 space-y-2">
          <p className="text-sm text-zinc-400">{d.jacket_no_donor_data}</p>
          <p className="text-xs text-zinc-600">
            {d.jacket_no_donor_check}{" "}
            ...
            <table className="w-full min-w-[520px] border-collapse text-sm">
              <thead>
                <tr className="border-b border-jacket-border bg-jacket-gray/30 text-left">
                  <th className="p-2 font-mono">{d.jacket_donor_col}</th>
                  <th className="p-2 font-mono">{d.jacket_category_col}</th>
                  <th className="p-2 font-mono">{d.jacket_amount_col}</th>
                  <th className="p-2 font-mono">{d.jacket_status_col}</th>
                </tr>
              </thead>
              <tbody>
                {donorsWithAmounts.map((donor) => (
                  <tr key={`${donor.name}-${donor.category}`} className="odd:bg-jacket-gray/20">
                    ...
                    <td className="p-2 text-zinc-400">{donor.confirmed ? d.jacket_confirmed : d.jacket_alleged}</td>
                  </tr>
                ))}
              </tbody>
            </table>
          ...
          {(showPartialNote || donorsNote) && (
            <div className="border border-amber-900/40 bg-amber-950/20 px-3 py-2 text-xs text-amber-400">
              {donorsNote
                ? donorsNote
                : coveragePct !== null
                  ? `${d.jacket_partial_note_pre} ${donorsWithAmounts.length} ${donorsWithAmounts.length !== 1 ? d.jacket_partial_note_donors : d.jacket_partial_note_donor} (${Math.round(coveragePct * 100)}% ${d.jacket_partial_note_of}). ${d.jacket_partial_note_see}`
                  : null}
            </div>
          )}
        </>
