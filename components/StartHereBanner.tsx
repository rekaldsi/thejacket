...
export default function StartHereBanner() {
  // Start as NOT dismissed — so SSR and initial client render match (no hydration mismatch)
  // Then immediately correct from sessionStorage after mount
  ...
  // If dismissed, render nothing (after hydration only — avoids flash)
  if (hydrated && dismissed) return null;

  return (
    <>
      {/* ── MOBILE: fixed bubble, zero flow impact ── */}
      {hydrated && !dismissed && (
      ...
      {/* ── DESKTOP: in-flow slim banner ──
          Rendered on SSR too so there's no layout shift.
          Uses opacity + pointer-events to hide without reflow once dismissed.
      ── */}
      <div
        className={`hidden md:block transition-all duration-300 overflow-hidden ${
          hydrated && dismissed
            ? "max-h-0 opacity-0 pointer-events-none py-0 mb-0"
